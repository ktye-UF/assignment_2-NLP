...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    sentVector = np.mean(wordVectors[indices, :], axis=0)  # calculate sentence features
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    grad = features.T.dot(grad) / N   # calculate gradient
    grad += regularization * weights  # gradient with regularization
    
    if N > 1:                         # choose maximum prob as our prediction
