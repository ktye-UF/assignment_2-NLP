...
def normalizeRows(x):
    ...
    x_sum = np.sqrt(np.sum(x**2, 1))    # equals to dividing |x|
    x /= np.reshape(x_sum, (-1, 1))
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    score = np.dot(predicted, outputVectors.T)    # forward prop           
    out = softmax(score)
    
    cost = np.sum(-y * np.log(out))               # cost for predict one
    
    dout = out - y                                # sensitivity 
    gradPred = np.dot(dout, outputVectors)        # gradient of predict one    
    grad = np.dot(dout.T, predicted)              # gradient of others
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    # function
    a_target = sigmoid(np.dot(predicted.reshape(-1), outputVectors[target].T))  # cost for target value
    ...
    for i in range(K):
        ...
        if j == target or (j in neg_samples):   # resample if sample happens to be itself
            i -= 1          
            continue
